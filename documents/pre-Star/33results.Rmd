...
The base model parameter estimates along with approximate asymptotic standard errors (SD column) are shown in Table \@ref{tab:params}. This table enumerates a total of 241 estimated parameters. These include one parameter for $R_0$, one parameter for female natural mortality, nine parameters describing growth, one parameter for survey catchability in the \gls{s-tri}, one parameter for extra variability for the pre-recruit survey, 75 parameters for length-based selectivity and blocks along with 22 parameters for sex-dependent selectivity and blocks, and 131 recruitment deviations.
...
**Add statement about sensitvity to having a full asymptotic fleet to teh above depending on results**
...
Catchability for surveys comparing observed to expected vulnerable biomass across all years was analytically solved for the \gls{s-wcgbt} and pre-recruit survey, and estimated for the \gls{s-tri}. The \gls{s-tri} was mirrored across the early and late period so catchability was the same. The values for catchability of the adult surveys are commensurate with surveys for a species that is partially inaccessible and caught on a subset of tows (Tables \ref{tab:wcgbts}, \ref{tab:triennial}). Catchability in regular space was calculated as 0.51 for the \gls{s-wcgbt}, 0.064 for the pre-recruit survey, and estimated to be 0.24 for the mirrored \gls{s-tri} (Table \ref{tab:params}). Additional fishery and survey index variability, which is process error added directly to each year's input standard deviation for the pre-recruit survey, was estimated within the model at 0.464. Fits to adult surveys were near enough to confidence intervals of the estimates of the survey that down-weighting the survey data by estimating additional variability was not warranted.
...
**Need to add these figures**
Pearson residuals plots by fishery fleet as well as mean age by year observed from the composition data are show in (Figures \ref{fig:age-pearson1}--\ref{fig:mean-age-fit12}). The range of ages is larger than the range of lengths, but patterns are comparable to those observed from the length composition fits. Patterns in age data also indicate a declining trend in mean age in the fishery starting in the early years (1980s) when data is available. The decline is best captured in the fit to the Oregon trawl ages, although the fit underestimates the decline in mean age. Fits to California trawl ages are sparse and to Washington trawl ages are overestimating observed data, but also indicate a decline in mean age. As with lengths, recreational mean ages suggest an increase in mean age in recent years. Patterns in mean length for the non-trawl and at-sea-hake fleets are generally uninformative.

**Need to add these figures**
Pearson residuals of conditional age-at-length compositions are shown for each survey (Figures \ref{fig:caal-pearson28}--\ref{fig:caal-pearson30}. These show variability in length at age over time, suggesting use of conditional-age-at-lengths data better captures model uncertainty than external estimates of growth. Positive residuals near the lower range of the data exist for the early \gls{s-tri} and recent years of the \gls{s-wcgbt}. These suggest the data indicates older male `r spp` at length than is being estimated in the model for these years, which is consistent with our comparison of externally estimated growth parameters for males being near the lower uncertainty range of parameter values estimated internal to the model. Despite these patterns, predictive plots where a well-fitting model has a predicted average and state deviation of age-at-length that is within the 95% predictive interval are available within the r4ss documentation available online. Predictive plots for all surveys generally exhibit this behavior, which provides no evidence of poor fit to conditional age-at-length data, and suggests that a model with time-constant growth provides a satisfactory approximation to the process governing age and length.  


**TO DO: I didn't get to this. Need to do this yet**
