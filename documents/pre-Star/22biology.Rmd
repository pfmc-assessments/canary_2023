...
within the 2015 assessment (Figure \ref{fig:compare_maturity}). *TO DO:
add data to figure*
...
`r spp` grow slower and achieve larger maximum sizes farther north, with
a break point estimated between Newport, OR and Coos Bay, OR
[@Brooks_thesis_2021]. However, estimating spatial growth differences
outside of the stock assessment model on the more numerous \gls{s-wcgbt}
data (1567 vs. `r sum(wcgbts.summary[,'N ages'])` samples; maximum age
of 51 vs. 65) indicated that although individuals captured north of Coos
Bay, OR did grow to slightly larger sizes than those captured south of
Coos Bay, particularly female individuals, the differences were
biologically minor and did not warrant the additional complexity in the
stock assessment model (Figure \ref{fig:spatial_AL}). Thus, growth was
assumed constant across the coast, as was assumed in the 2015 benchmark
assessment. \*\*TODO: These values are wrong, Brian I am not sure where
the right values are\*\* Externally estimated von-Bertalanffy growth
parameters for `r spp` using \gls{s-wcgbt} data were as follows:
$L_\infty$ = `r model$MGparmAdj$L_at_Amax_Fem_GP_1[1]` and $k$ =
