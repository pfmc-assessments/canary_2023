...
Commercial length- and marginal age-frequency distributions were calculated for each fleet for which observations were available. Biological length and age data were extracted from the \gls{pacfin} Biological Data System on March 28, 2023. Biological data were divided into trawl and nontrawl fleets for each state according to the fleet structure used for this assessment. There was a mix of biological length samples with and without sex information but the majority of samples included information on sex.

During data exploration it was noted that data previously labelled as special project samples from Oregon included samples appropriate to represent dynamics from the fishery. The majority of these occurred prior to 1987, and therefore special project samples from Oregon up through 1986 were retained for composition data.  
...
The magnitude of sampling for each fleet and year are given in TABLES and FIGURES, and show the different magnitudes of sampling across time and between state agencies. These tables show both the total number of fish samples as well as the number of trips or hauls by year and state over which samples were obtained. 
