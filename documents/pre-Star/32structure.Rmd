...
where able, and assuming natural mortality to be age-invariant, as
opposed to increasing at age.
...
change in terms of effects on model results is with the change in
...
linear ramp in female natural mortality. Assumptions to this choice are
...
entire modeled area, early explorations indicated some instability in
the spatial model when unmirroring and expanding selectivity. Through
bridging (described below) we show that the change in population
structure does not appear to have a large effect on model results,
especially relative to other changes in natural mortality and the
...
the available data in specific ways. Fleets were assumed to have the
same selectivity across states in the 2015 assessment. We allowed
selectivity estimates to be informed by the dynamics in waters where
they fish, although we have mirrored some fleets where we believe it is
appropriate to do so. We have also increased the number of selectivity
blocks for the non-trawl and recreational fisheries, which we believe is
...
improves fits to data, but also allows dynamics between male and female
natural mortality to align better with observed sex-ratios.
Sensitivities to choice about selectivity are described in Section
\@ref(sensitivities).
...
patterns observed in the non-foreign fishery. We also assigned data from
...
have been provided using updated and more reproducible approaches.
Significant effort was made by the \gls{nwfsc} ageing lab in providing
age reads for new and past `r spp`. These allow age composition data for
...
confirming the sample sizes of composition data do not exceed the number
...
sample size above the number of fish sampled. Weights for
...
be inferior to the Francis method through simulation
...
enforce that main period deviation sum to one. Other than these
parameters and those that are fixed, we do not include any other
informative priors or constraints on parameters.
...
available, and selectivity is mirrored to the domestic trawl fishery for
...
natural mortality were estimated. We found that allowing sex-dependent
selectivity in parameter 4, which governs how steeply selectivity
declines at larger sizes, the fit to the data improved. Sex-dependence
...
In this assessment the logarithm of $R_0$ (unfished recruitment) was estimated, while steepness
