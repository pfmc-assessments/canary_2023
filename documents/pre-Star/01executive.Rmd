...
There are two measures of exploitation status (Table \ref{tab:spr-es}). The target harvest rate is determined by 1-SPR or "fishing intensity." SPR is the spawning potential ratio, or percent of unfished spawning output that would result at equilibrium from fishing at a given rate. Because the target is based on 1-SPR, a value of 0 means no fishing is occurring (unfished spawning output would result at equilibrium from fishing at the current rate), and a value of 1 means all spawning fish would be killed before spawning at equilibrium, and the population would collapse. The second measure is the exploitation rate, or total dead catch divided by total age `r model$summary_age`+ biomass.
