...
`r Spp` was most recently assessed in 2015 using an age-structured population model that allowed for spatial differences in recruitment deviations and depletion by state. The current assessment uses an areas-as-fleets approach to account for different sizes and ages of fish available in each state, but returns to a coastwide model configuration. 

This assessment includes catches from five fleets, each of which is divided across three states, adult fishery-independent indices from the \gls{s-wcgbt} and \gls{s-tri}, age and length data from the fishery and adult surveys, and a pre-recruit survey. It extends all of these data sets from the previous assessment. In addition, it includes a maturity curve based on newly analyzed ovaries and age data from recently aged otoliths from the 1986 \gls{s-tri}.
