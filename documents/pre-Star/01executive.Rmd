...
The management target for `r spp` is 40% of unfished spawning output, or $B_{40\%}$. This is associated with an annual exploitation rate of `r round(model$derived_quants['annF_Btgt', 'Value'], 4)` yr^-1^ (95% confidence interval, `r stringr::str_flatten(round(qnorm(c(0.025, 0.975), model$derived_quants['annF_Btgt', 'Value'], model$derived_quants['annF_Btgt', 'StdDev']), 4), collapse = '-')`) and yield of `r round(model$derived_quants['Dead_Catch_Btgt', 'Value'], 0)` (`r stringr::str_flatten(round(qnorm(c(0.025, 0.975), model$derived_quants['Dead_Catch_Btgt', 'Value'], model$derived_quants['Dead_Catch_Btgt', 'StdDev']), 0), collapse = '-')`). The target fishing intensity is SPR=0.5, which leads to an equilibrium spawning output of `r round(model$derived_quants['SSB_SPR', 'Value'], 0)` million eggs (`r stringr::str_flatten(round(qnorm(c(0.025, 0.975), model$derived_quants['SSB_SPR', 'Value'], model$derived_quants['SSB_SPR', 'StdDev']), 0), collapse = '-')`). Table \ref{tab:ref-pts-es} contains a full list of model estimates and 95% confidence intervals of target spawning output, target 1-SPR, target exploitation rate, and target yield for targets based on spawning depletion, 1-SPR, and the model estimate of MSY (maximum sustainable yield), as well as key population quantities at unfished levels and their current values.
...
Total mortality is generally well below the annual catch limit (Table \ref{tab:acls-es}). However, from 2013--2016 the stock was designated as overfished. During that time, attainment averaged around 60%. In 2015 the full \gls{acl} was nearly achieved (93% attainment). Beginning in 2017, the \gls{acl} increased more than tenfold with the lifting of the overfished designation. Since that time, catches have increased and attainment has averaged around 40%.
...
1.  Continued research into the rationale for skewed sex ratios and empirical studies to estimate natural mortality rates, as well as basic life history research for fecundity and growth.

2.  The \gls{s-wcgbt} has low encounter rates with `r spp` in part because it has limited access to rocky habitat. We recommend exploration of a non-trawl coast-wide fishery-independent indices.
