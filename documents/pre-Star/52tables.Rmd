...
readr::read_csv(here('data/canary_total_removals.csv')) |> 
  ...
  knitr::kable(caption = 'Total removals (mt) of canary rockfish for the trawl (TWL), non-trawl (NTWL) used in the assessment model. See text for description of sources',
               label = 'catches1', 
               format = 'latex',
               col.names = c("Year", 
...
readr::read_csv(here('documents/figures/sensitivities/data_table.csv')) |>
  ...
  table_format(caption = 'Estimates of likelihood components and major model parameters for data sensitivities. Likelihood is negative log-likelihood, where lower values indicate better fit to data. Because of differences in input data, likelihoods for the `Float Q` model are not comparable.',
             label = "data-sens",
             row.names = FALSE,
             landscape = TRUE,
             custom_width = TRUE,
             align = "l",
             col_to_adjust = c(2,3,4,5,6,7,8),
             width = c("1.27cm", "1.676cm", "1.676cm", "1.676cm", "1.27cm", "1.27cm", "1.905cm"))
```
