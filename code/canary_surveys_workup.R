load(here('data/catch_canary rockfish_NWFSC.Combo_2022-11-03.rda'))
load(here('data/catch_canary rockfish_Triennial_2022-11-03.rda'))
...
age_combo <- dplyr::select(wcgbts_bio, Year, Length_cm, Sex, Age_years, Latitude_dd, Longitude_dd) %>%
  ...
  dplyr::mutate(is_south_cb = dplyr::if_else(Latitude_dd < 43.3672,
                                        TRUE, FALSE),
                is_south_ca = dplyr::if_else(Latitude_dd < 42,
                                             TRUE, FALSE),
                is_south_wa = dplyr::if_else(Latitude_dd < 46.25,
                                             TRUE, FALSE))
...
split_sex <- nls(Length_cm ~ (linf+linf_adj*(Sex == 'M'))*(1-exp(-k*(Age_years-t0))), data = age_combo, 
...
split_region_ca <- nls(Length_cm ~ (linf+linf_adj*is_south_ca)*(1-exp(-k*(Age_years-t0))), data = age_combo, 
                 start = list(linf = 55, linf_adj = 0, k = 0.3, t0 = 0)) 

split_region_cb <- nls(Length_cm ~ (linf+linf_adj*is_south_cb)*(1-exp(-k*(Age_years-t0))), data = age_combo, 
                       start = list(linf = 55, linf_adj = 0, k = 0.3, t0 = 0)) 
# results in linf_adj = -0.557

split_region_wa <- nls(Length_cm ~ (linf+linf_adj*is_south_wa)*(1-exp(-k*(Age_years-t0))), data = age_combo, 
                       start = list(linf = 55, linf_adj = 0, k = 0.3, t0 = 0)) 

AIC(split_region_ca, split_region_cb, split_region_wa, coastwide)
...
vbgf.df <- expand.grid(Age_years = seq(0, 60, length.out = 100),
                       is_south_ca = c(TRUE, FALSE)) %>% 
  dplyr::as_tibble() %>%
  dplyr::mutate(Length_cm_s_args = lapply(Age_years, function(x) c(x = x, as.list(coef(split_region_ca)))),
                Length_cm_n_args = lapply(Age_years, function(x) c(x = x, as.list(coef(split_region_ca)[-2]))),
                Length_cm_s = sapply(Length_cm_s_args, do.call, what = vbgf),
                Length_cm_n = sapply(Length_cm_n_args, do.call, what = vbgf),
                Length_cm = ifelse(is_south_ca, Length_cm_s, Length_cm_n))

# scatter plot with fitted lines, colored by CA vs OR/WA
age_combo %>%
  ggplot(aes(x = Age_years, y = Length_cm, col = is_south_ca)) +
...
age_combo %>%
  ...
  geom_function(data = transform(age_combo, is_south_ca = TRUE), col = 'red',
                fun = vbgf, args = as.list(coef(split_region_ca))) +
  geom_function(data = transform(age_combo, is_south_ca = FALSE), col = 'red',
                fun = vbgf, args = as.list(coef(split_region_ca)[-2])) +
  facet_wrap(~is_south_ca, 
