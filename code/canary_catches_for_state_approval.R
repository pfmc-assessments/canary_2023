##############################################################################################################
#
# 	Purpose: Output Canary Rockfish Landings and Discards for state verification
#
#   Created: Apr 12, 2023
#			  by Brian Langseth 
#
#   This duplicates a lot of what is in canary_catches_for_model
#     but is simplified. Used for approving catch times series
#     since the states want landings and discards separate for
#     the commercial fleets (and I split for WA rec)
#
#   Output from this script will be confidential
#
##############################################################################################################
...
##
#Recreational data
#This is output from canary_catches.rec.R for splitting out for state approval
##

rec <- utils::read.csv(file = file.path(git_dir, "data", "canary_rec_catch_splitOut.csv"), header = TRUE)
#Extend rec to incorporate CA historical time period
rec <- rbind(data.frame("Year" = c(1928:1966), 
                        "wa_N" = 0, "wa_N_dis" = 0,
                        "or_MT" = 0, "ca_MT" = 0), rec)
rec[is.na(rec)] <- 0
...
land_names <- c("NTWL.C","NTWL.O","NTWL.W","TWL.C","TWL.O","TWL.W")
disc_names <- c("NTWL.C.dis","NTWL.O.dis","NTWL.W.dis","TWL.C.dis","TWL.O.dis","TWL.W.dis")
removals <- data.frame("Year" = 1892:2022, 
                     "NTWL.C" = 0, "NTWL.C.dis" = 0,
                     "NTWL.O" = 0, "NTWL.O.dis" = 0,
                     "NTWL.W" = 0, "NTWL.W.dis" = 0,
                     "TWL.C" = 0, "TWL.C.dis" = 0,
                     "TWL.O" = 0, "TWL.O.dis" = 0,
                     "TWL.W" = 0, "TWL.W.dis" = 0)
...
##
#Add discards and pacfin landings >=2000
##

#Add 2002-2021 GEMM estimates
removals[which(removals$Year %in% pacfin$LANDING_YEAR), c("Year",land_names)] <- data.frame(pacfin)
removals[is.na(removals)] <- 0
removals[which(removals$Year %in% gemm_discard$Year),disc_names] <- round(gemm_discard[,c("ca_ntwl","or_ntwl","wa_ntwl","ca_twl","or_twl","wa_twl")],3)
...
dis_rat_late <- colSums(gemm_discard[which(gemm_discard$Year %in% c(2019:2021)),c("ca_ntwl","or_ntwl","wa_ntwl","ca_twl","or_twl","wa_twl")]) /
  (colSums(removals[which(removals$Year %in% c(2019:2021)),land_names]) + colSums(removals[which(removals$Year %in% c(2019:2021)),disc_names]))
...
#Although last assessment used <1987 only, Ali has updated the proportions of nominal canary
...
removals$rec.O <- 0
removals$rec.W.N <- 0
removals$rec.W.N.dis <- 0
removals[removals$Year %in% rec$Year, c("rec.W.N","rec.W.N.dis","rec.O","rec.C")] <- rec[,-1]
...
removals$rec.W.mt.2.dis <- 0
removals[removals$Year %in% c(1967:2022),]$rec.W.mt.2 <- wa_rec_avgW$avgW2*removals[removals$Year %in% c(1967:2022),"rec.W.N"]
removals[removals$Year %in% c(1967:2022),]$rec.W.mt.2.dis <- wa_rec_avgW$avgW2*removals[removals$Year %in% c(1967:2022),"rec.W.N.dis"]
...
# ##
# xx <- googledrive::drive_create(name = 'CONFIDENTIAL_canary_removals_forStateApproval_June8',
