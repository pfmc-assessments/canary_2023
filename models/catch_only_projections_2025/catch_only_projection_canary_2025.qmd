...
params:
  ...
  dir_new3: "../models/catch_only_projections_2025/2025_2026project_part3"
...
model3 <- r4ss::SS_output(
  dir = here::here("models", "catch_only_projections_2025", "2025_2026project_part3"),
  #dir = params$dir_new3,
  verbose = FALSE,
  printstats = FALSE,
  hidewarn = TRUE)
...
table_caption2 <- glue::glue("Original {assess_year} projection and alternative catch-only projection in which the 2026 assumed catches are set to 88% of the ACL (522 = 0.88 * 593) rather than fixed at the values calculated by the GMT based on the current harvest specifications. See caption of table above for additional details.")

table_caption3 <- glue::glue("Original {assess_year} projection and alternative catch-only projection in which the 2026 assumed catches are set to 88% of the ABC (551 = 0.88 * 626) rather than fixed at the values calculated by the GMT based on the current harvest specifications. See caption of table above for additional details.")
```
...
Two alternative projections are provided. In @tbl-proj2, the assumed catch in 2026 is based on the ACL from a catch-only projection through 2025 rather than the current harvest specifications. In @tbl-proj3, the assumed catch in 2026 is based on the ABC from a catch-only projection through 2025 rather than the current harvest specifications, thereby removing the effect of the 40-10 rule for 2026. An assumption of 88% attainment for 2026 catches, as estimated by the GMT, was used in both cases.
...
wide_table2$ACL[wide_table2$Year == 2026] <- 593 #Hard coded addition based on model 1a
...
wide_table3$ACL[wide_table3$Year == 2026] <- 626 #Hard coded addition based on model 1a ABC
```
